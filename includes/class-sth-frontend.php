...
class STH_Frontend {
	...
	public function output_dynamic_css() {
		...
		$selector = ( ! empty( $options['apply_globally'] ) || empty( $options['custom_selector'] ) ) ? '' : esc_html( $options['custom_selector'] );
		...
		// Helper closure for backgrounds
		$get_bg = function($type, $color1, $color2, $alpha = '') {
			$c1 = $color1 . $alpha;
			$c2 = $color2 . $alpha;
			if ($type === 'linear') {
		...
		$y_track_bg = $get_bg($options['y_track_gradient_type'], $options['y_track_color'], $options['y_track_color_2'], $y_track_alpha);
		$y_thumb_bg = $get_bg($options['y_thumb_gradient_type'], $options['y_thumb_color'], $options['y_thumb_color_2']);
		$y_thumb_hover_bg = $get_bg($options['y_thumb_gradient_type'], $options['y_thumb_hover_color'], $options['y_thumb_hover_color_2']);

		$x_track_bg = $get_bg($options['x_track_gradient_type'], $options['x_track_color'], $options['x_track_color_2'], $x_track_alpha);
		$x_thumb_bg = $get_bg($options['x_thumb_gradient_type'], $options['x_thumb_color'], $options['x_thumb_color_2']);
		$x_thumb_hover_bg = $get_bg($options['x_thumb_gradient_type'], $options['x_thumb_hover_color'], $options['x_thumb_hover_color_2']);

		$corner_bg = $get_bg($options['corner_gradient_type'], $options['corner_color'], $options['corner_color_2'], $corner_alpha);

		// Glassmorphism
		$glass_css = '';
		if ( ! empty( $options['glassmorphism'] ) ) {
			$glass_css = "backdrop-filter: blur(10px) !important; -webkit-backdrop-filter: blur(10px) !important;";
		}

		// Animation
		$animation_css = '';
		if ( ! empty( $options['animation'] ) ) {
			$animation_css = "animation: sth-pulse 2s infinite !important;";
		}
		...
		if ( ! empty( $options['neon_glow'] ) ) {
			// Using inset so it doesn't get clipped by the scrollbar container bounds
			$glow_css = "box-shadow: inset 0 0 10px {$options['glow_color']} !important;";
			$glow_css_hover = "box-shadow: inset 0 0 15px {$options['glow_color']} !important;";
		}
		...
		<style id="sth-custom-scrollbar-css">
			...
			<?php echo $selector; ?>::-webkit-scrollbar-track:vertical {
				background: <?php echo esc_html( $y_track_bg ); ?> !important;
				<?php echo $glass_css; ?>
			}
			<?php echo $selector; ?>::-webkit-scrollbar-track:horizontal {
				background: <?php echo esc_html( $x_track_bg ); ?> !important;
				<?php echo $glass_css; ?>
			}

			/* Thumb X and Y */
			<?php echo $selector; ?>::-webkit-scrollbar-thumb:vertical {
				background: <?php echo esc_html( $y_thumb_bg ); ?> !important;
				border-radius: <?php echo esc_html( $options['y_thumb_radius'] ); ?>px !important;
				<?php echo $glass_css; ?>
				<?php echo $glow_css; ?>
				<?php echo $animation_css; ?>
			}
			<?php echo $selector; ?>::-webkit-scrollbar-thumb:horizontal {
				background: <?php echo esc_html( $x_thumb_bg ); ?> !important;
			...
			<?php echo $selector; ?>::-webkit-scrollbar-thumb:vertical:hover {
				background: <?php echo esc_html( $y_thumb_hover_bg ); ?> !important;
				<?php echo $glass_css; ?>
				<?php echo $glow_css_hover; ?>
			}
			<?php echo $selector; ?>::-webkit-scrollbar-thumb:horizontal:hover {
				background: <?php echo esc_html( $x_thumb_hover_bg ); ?> !important;
				<?php echo $glass_css; ?>
				<?php echo $glow_css_hover; ?>
			}
			...
			<?php echo $selector; ?>::-webkit-scrollbar-corner {
				background: <?php echo esc_html( $corner_bg ); ?> !important;
			}
